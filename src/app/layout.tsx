        ...
        <ThemeProvider>
          <SessionProviderWrapper session={session}>
            ...
            <nav className="bg-blue-900 dark:bg-slate-900 text-white p-4 shadow-md border-b border-transparent dark:border-slate-800">
                  ...
                  {((session?.user as any)?.role === "ADMIN" || (session?.user as any)?.role === "VORSTAND") && (
                    <>
                      <Link href="/admin/postfach" className="text-amber-400 hover:text-blue-200 dark:hover:text-blue-300 font-medium transition-colors">
                        ...
                        {unreadCount > 0 && (
                          <span className="bg-red-500 text-white text-[11px] font-extrabold px-1.5 py-0.5 rounded-full leading-none shadow-sm">
                            {unreadCount}
                          </span>
                        )}
                      </Link>
              ...
              <div className="bg-white/60 dark:bg-slate-900/60 backdrop-blur-[0.5px] border border-white/50 dark:border-slate-700/50 p-6 sm:p-8 rounded-2xl shadow-2xl transition-all duration-300">
                {children}
              </div>
              </main>
